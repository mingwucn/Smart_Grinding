...
required_packages = ['scienceplots', 'librosa', 'nptdms', 'natsort', 'dill', 'openpyxl', 'seedir']
...
from MyCustomModels import MyCustomGrindingPredictor as GrindingPredictor # Use the custom predictor
from GrindingData import GrindingData # Still need original GrindingData for physics loading
from MyDataset import project_dir, allowed_input_types # Import allowed_input_types from original MyDataset
...
def load_trained_model(model_type="ae_features", fold=0):
    ...
    try:
        ...
        print(f"Loaded model: {model_filename} (using MyCustomGrindingPredictor's load_state_dict)")
...
def generate_predictions_for_model(model_type):
    """
    Generate predictions for a single model type.
    Returns a dictionary with predictions for the model type.
    """
    predictions_dict = {}
    ...
    # Load dataset based on model type using the custom dataset getter
    try:
        dataset = get_custom_dataset(input_type=model_type, dataset_mode="classical")
    except Exception as e:
    ...
    with torch.no_grad():
        for i in range(len(dataset)):  # Use all samples
            try:
                item = dataset[i]
    ...
    if len(predictions) > 0:
        ...
        print(f"MAE: {np.mean(np.abs(np.array(true_values) - np.array(predictions))):.3f} um")
    else:
...
def plot_time_series_with_physics(true_values, predictions, bdi_values, model_type, sample_indices=None):
    ...
    # Add MAE annotation
    ax.text(0.02, 0.98, f'MAE = {mae:.2f} um', 
...
def main():
    ...
    for model_type in allowed_input_types:
        predictions_for_type = generate_predictions_for_model(model_type)
        if predictions_for_type:
            all_predictions_data.update(predictions_for_type)
    ...
    for model_type, data in all_predictions_data.items():

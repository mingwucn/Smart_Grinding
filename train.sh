...
for input_type in "${allowed_input_types[@]}"; do
    while true; do
        ...
        python ./trainer.py --epochs 10 --batch_size 2 --learning_rate 1e-5 --model_name "$input_type" --input_type "$input_type" --verbose_interval 0 2>&1 | tee >(while IFS= read -r line; do echo "$(date '+%Y-%m-%d %H:%M:%S') $line" >>"log/train_${input_type}.txt"; done)
